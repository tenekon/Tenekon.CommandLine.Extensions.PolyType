...
internal sealed class CommandLineNamer(
    ...
    CommandLineNamer? parent = null)
{
        ...
        NameCasingConvention casing)
    {
        ...
        if (!_nameAutoGenerate.HasFlag(flag))
            return fallback;
    ...
    private static string ToTitleCase(string name)
    {
        ...
        return string.Concat(
            words.Select(word => word.Length == 0
                ? string.Empty
                : char.ToUpperInvariant(word[index: 0]) + word.Substring(startIndex: 1).ToLowerInvariant()));
    }
    ...
    private static string ToPascalCase(string name)
    {
        ...
        return string.Concat(
            words.Select(word => word.Length == 0
                ? string.Empty
                : char.ToUpperInvariant(word[index: 0]) + word.Substring(startIndex: 1)));
    }
