...
public class CommandHandlerFactoryTests
{
    ...
    public void TryCreateHandler_BothRunAndRunAsync_PrefersAsync()
    {
        ...
        handler.Invoke(parseResult, null);
    ...
    public void Invoke_RunReturnsInt_ReturnsCode()
    {
        ...
        var code = handler.Invoke(parseResult, null);
    ...
    public async Task InvokeAsync_RunAsyncReturnsInt_ReturnsCode()
    {
        ...
        var code = await handler.InvokeAsync(parseResult, null, CancellationToken.None);
    ...
    public async Task InvokeAsync_RunAsyncWithCancellationToken_ResolvesToken()
    {
        ...
        await handler.InvokeAsync(parseResult, null, cts.Token);
    ...
    public void Invoke_RunWithServiceParameter_ResolvesDependency()
    {
        ...
        var (handler, parseResult, _) = CreateHandler<RunWithServiceCommand>(["--trigger"], settings, provider);

        handler.Invoke(parseResult, provider);
    ...
    public void Invoke_RunWithContextAndService_ResolvesDependencyAndContext()
    {
        ...
        services.AddSingleton(new DiDependency("service"));
        var provider = services.BuildServiceProvider();
        var (handler, parseResult, _) = CreateHandler<RunWithContextAndServiceCommand>(
            ["--trigger"],
            settings,
            provider);
    ...
    public async Task InvokeAsync_RunAsyncWithServiceAndCancellationToken_ResolvesDependencyAndToken()
    {
        ...
        using var cts = new CancellationTokenSource();
        cts.Cancel();
        var (handler, parseResult, _) = CreateHandler<RunAsyncWithServiceAndTokenCommand>(
            ["--trigger"],
            settings,
            provider);

        await handler.InvokeAsync(parseResult, provider, cts.Token);
    ...
    public void Invoke_RunWithRequiredServiceMissing_Throws()
    {
        ...
        Should.Throw<InvalidOperationException>(() => handler.Invoke(parseResult, null));
    }
    ...
    {
        HandlerLog.Reset();
        var settings = new CommandLineSettings { ShowHelpOnEmptyCommand = true };
    ...
    private static (CommandHandler Handler, ParseResult ParseResult, StringWriter Output) CreateHandler<TCommand>(
        ...
        IServiceProvider? serviceProvider = null) where TCommand : IShapeable<TCommand>
    {
        ...
        var graph = CommandGraphBuilder.Build(shape, shape.Provider, settings, serviceProvider);
        var parseResult = graph.RootCommand.Parse(args);
        var handler = CommandHandlerFactory.TryCreateHandler(
            shape,
            graph.BindingContext,
            settings,
            graph.RootDescriptor);
